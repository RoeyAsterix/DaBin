...
Release 0.3.16 replaces the text Daily/Weekly segment with matching purple `1` and `7` calendar icons. They use the same symbol size, circular selected/hover states, pressed feedback, keyboard focus and short delayed tooltips as the other header icons, while keeping full **Daily view** and **Weekly view** accessibility names. All 30 optimized Release suites, 50 release renders, a real pointer/accessibility check, the ARM64 build and isolated update-package QA passed. Public publication and the installed-app update are the remaining release steps.
