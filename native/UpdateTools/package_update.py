...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='dabin-update-package-') as temporary_name:
        ...
        (package / 'README.txt').write_text(f'''DaBin {version} ({build_number}) update for this Apple Silicon Mac\n\nDouble-click “DaBin Update.app”, then choose Update. It will:\n\n1. Ask DaBin to quit normally.\n2. Verify the ARM64 Release application and its signature.\n3. Back up the existing app under ~/Applications/.DaBinBackups/.\n4. Replace ~/Applications/DaBin.app and verify the installed copy.\n5. Reopen DaBin on Daily.\n\nYour captures are stored separately in DaBin's sandbox container. The updater\ndoes not read, copy, move or delete that archive. If DaBin has an unsaved edit\nor removal in progress, finish it and run the updater again.\n\nThis locally ad-hoc signed updater is for the current Mac. It is not a Mac App\nStore installer, notarized public distribution or Apple-approved update channel.\nNo security setting should be disabled.\n''')
        if args.guide:
