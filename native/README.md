...
Release 0.3.13 makes the primary-action and filter rows geometrically identical while preserving their order and behavior. Each row occupies 280 × 34 points; every control uses a 15-point symbol in an 18-point canvas, a 40 × 34-point target and a 30-point circular selected or hover surface. The five primary actions distribute between the same outer edges as the six filters. The public release is available through Settings, and the installed 0.3.12 build successfully updated itself to 0.3.13 through that flow.
