...
Release 0.3.4 supersedes 0.3.3. The 0.3.3 updater tried to pass the verified package as LaunchServices arguments, which macOS ignores when the caller is sandboxed; the one-use document handoff removes that dependency. If an existing 0.3.3 installation reports a missing `DaBin.app` during update, use the release ZIP fallback below to install 0.3.4.
