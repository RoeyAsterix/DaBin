...
See [APP_STORE_READINESS.md](APP_STORE_READINESS.md) for the current audit and release gates. The source includes a bundled privacy policy, privacy manifest, Productivity category, export-compliance declaration, Store-only compile boundary, safe archive location check, and recursive quarantine gate. [Final local evidence](../docs/qa/app-store-preflight-2026-09-24/README.md) and [1440 × 900 screenshot drafts](../docs/app-store/screenshots/1440x900/README.md) are tracked. Submission still needs full Xcode, an Apple Developer team, release signing, validation, real support contact details, and App Store Connect metadata. `python3 scripts/app_store_preflight.py --static-only` checks source packaging; the release preflight intentionally fails while owner inputs are missing.
