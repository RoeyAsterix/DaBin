...
A native, local daily capture board. Daily opens once on first launch; after dismissal, DaBin keeps only a compact menu bar status visible while idle. By default, reach any screen corner to reveal the metallic purple robot. On a Mac with a built-in camera island, you can instead let it peek out below the island. Drop onto its body or hover over it and press **Control-V** or **⌘V**. The robot watches the pointer, welcomes incoming content, chews while saving and reacts to the result. Double-click it to open Daily. The floating board starts at 75% opacity and can be adjusted in Settings.

## Run

Open **DaBin.app in your personal Applications folder** (`~/Applications/DaBin.app`) on this Apple Silicon Mac. A copy also ships at [build/DaBin.app](build/DaBin.app). The first launch opens Daily once. Later launches keep the board hidden until you use a screen corner, reopen the running app, or choose **Open Daily** from the menu bar. There is no permanent Dock icon, separate drop zone, or paste composer. The menu bar item shows Auto Capture state and provides Pause/Resume, Settings, and Quit.
...
- In the **Settings → Appearance**, toggle Dark mode and adjust the board transparency from 35% to 100% opacity. Under **Theme color**, choose Purple, Blue, Teal, Green, Rose or Amber, or pick a custom color. Changes apply immediately and are remembered on this Mac. **Reset** returns the accent to Purple. When macOS Reduce Transparency is enabled, DaBin temporarily uses a solid surface and explains the override in Settings.
...
- Double-clicking the robot expands its body into the existing app view in 1.15 seconds. The head, hands and feet stay outside the live content. X, Escape and Command W reverse the transformation in 0.42 seconds. Reduced motion uses a 0.14-second fade with a static frame.
...
See [APP_STORE_READINESS.md](APP_STORE_READINESS.md) for the current audit and release gates. The source includes a bundled privacy policy, privacy manifest, Productivity category, export-compliance declaration, Store-only compile boundary, safe archive location check, and recursive quarantine gate. Apple Developer Team `8QG4967CSU` and Bundle ID `com.dabin.mac` are stored in `Config/AppStoreSigning.json` and applied to the generated Release target. [Final local evidence](../docs/qa/app-store-preflight-2026-09-24/README.md) and [1440 × 900 screenshot drafts](../docs/app-store/screenshots/1440x900/README.md) are tracked. Submission still needs full Xcode, release signing assets, Bundle ID ownership confirmation, validation, real support contact details, and App Store Connect metadata. `python3 scripts/app_store_preflight.py --static-only` checks source packaging; the release preflight currently stops when full Xcode is unavailable.
