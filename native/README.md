...
Release 0.3.14 adds compact custom tooltips to all five primary actions and six filters. Short labels appear after a 220 ms hover delay or when an icon receives keyboard focus. A click dismisses the cue, and it clears after pointer and focus leave. The controls retain full accessibility names and the two 280 × 34-point rows keep their existing geometry. All 30 optimized Release suites, 46 production-view renders, the ARM64 build and isolated update-package QA passed; public publication and installed-app verification are the remaining release steps.
