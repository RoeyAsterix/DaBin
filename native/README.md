...
Release 0.3.16 replaces the text Daily/Weekly segment with two matching purple calendar icons: **1** for Daily and **7** for Weekly. Each has a 40 × 34-point target, selected and hover circle, press animation, focus ring, short tooltip and complete accessibility name. The 80-point pair is narrower than the old 92-point segment and remains unclipped in the 380-point Weekly header. All 30 optimized Release suites, 50 release renders, a real pointer/accessibility check, ARM64 build and isolated update-package QA passed. Public publication and the installed-app update are the remaining release steps.
