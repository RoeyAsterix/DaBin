...
Release 0.3.17 adds a complete **Quit DaBin** action to Settings and verifies that accepted termination stops every background capture monitor. All 30 optimized Release suites and 50 release renders passed. The same verified ARM64 app is published in both a one-click installer/update package and a separate manual Apple Silicon package. The installed 0.3.16 app completed its public update, the standalone package was separately installed and launched, and the Settings quit control left no DaBin process running. All 33 archive files, preferences, originals, imports, and Desktop links remained unchanged.

For the recommended path, extract `DaBin-<version>-Update.zip`, open **DaBin Update.app**, then choose **Install** or **Update**. For a manual installation, extract `DaBin-<version>-AppleSilicon.zip` and move **DaBin.app** to `~/Applications`. Keep only that copy so the in-app updater replaces the same application later.
