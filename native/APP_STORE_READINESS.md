...
**Result: LOCAL QA PASSED; submission remains blocked by account, Xcode, signing, and listing prerequisites.**
...
This source is not yet a distribution candidate. Full Xcode is absent, Command Line Tools are selected, `DABIN_DEVELOPMENT_TEAM` is unset, and there are no signing identities. The owner must enroll, reserve or confirm `com.dabin.mac`, provide the Team ID and Apple Distribution signing, publish support contact details, confirm publisher/copyright information, complete App Store Connect metadata and privacy answers, archive, validate in Organizer, and test the signed build on macOS 14 and current macOS. The three prepared 1440 × 900 screenshot drafts use fictional data and must be compared with that signed archive before upload.
...
The 0.3.18 source moves the existing user-initiated update controls into the first Settings card and adds a trusted latest-release link for direct builds. It does not add automatic network activity; update checks and downloads remain explicit. The final build-45 static source/package preflight passed all 21 checks; Store release preflight retains the same two machine/account prerequisites:

- `../docs/qa/app-store-preflight-2026-09-24/app-store-preflight-static.log`: all 21 source and packaging checks passed.
- `../docs/qa/app-store-preflight-2026-09-24/app-store-preflight-environment.log`: release preflight remains blocked by the Apple Developer Team ID and full Xcode. The configured GitHub policy/support URLs returned HTTP 200 during this audit; their content and continuing reachability remain owner responsibilities.
