...
The remaining sections retain the detailed review boundary and release workflow. Passing local checks does not guarantee Apple approval.
...
| PARTIAL | Manual and automatic capture controls | Manual clipboard reads remain tied to paste. Auto Capture is a separate opt-in setting that defaults off, takes a clipboard baseline before considering later changes, uses a user-selected screenshot folder, and stops its observers immediately when paused or disabled. The persistent menu bar item now shows enabled, paused and attention states and provides Pause/Resume. DaBin and common password managers are excluded by default, but source-app attribution is best effort and cannot guarantee origin. The final signed candidate still needs live first-launch, enable, pause, disable, relaunch, exclusion and permission-revocation review. Robot reveal needs no camera, Accessibility or Screen Recording permission. |
...
| BLOCKED | App Store signing and validation | Team `8QG4967CSU` is configured for the Release target and automatic signing. The inspected development app remains ad-hoc signed and has no TeamIdentifier because this Mac has no Apple signing identity or provisioning profile. A registered Bundle ID, Xcode-managed signing assets, archive export and successful Xcode distribution validation remain necessary. `com.dabin.mac` is configured locally; availability/ownership has not yet been confirmed in the account. [Distribution signing for macOS](https://developer.apple.com/documentation/xcode/creating-distribution-signed-code-for-the-mac/), [App Store provisioning](https://developer.apple.com/help/account/provisioning-profiles/create-an-app-store-provisioning-profile) |
...
| BLOCKED | App Store Connect release information | No App Store Connect record or account was accessed. Three dimension-valid screenshot drafts now exist, but the signed archive still needs comparison and upload. Publisher identity, support and privacy URLs, description, age-rating answers, privacy answers, review contact/notes, availability/pricing, and applicable export-compliance answers need owner confirmation and submission validation. Do not submit placeholder contact details. [Platform version information](https://developer.apple.com/help/app-store-connect/reference/app-information/platform-version-information), [App privacy details](https://developer.apple.com/app-store/app-privacy-details/) |
...
The 0.3.18 source moves the existing user-initiated update controls into the first Settings card and adds a trusted latest-release link for direct builds. It does not add automatic network activity; update checks and downloads remain explicit. The original build-45 evidence passed all 21 static checks. After enrollment, the current preflight adds five signing-configuration checks and passes all 26; the only local environment blocker is full Xcode:

- `../docs/qa/app-store-preflight-2026-09-24/app-store-preflight-static.log`: all 21 source and packaging checks passed.
- `../docs/qa/app-store-preflight-2026-09-24/app-store-preflight-environment.log`: historical pre-enrollment evidence records the former Team ID and Xcode blockers. The current Team ID check passes; full Xcode remains unavailable. The configured GitHub policy/support URLs returned HTTP 200 during this audit; their content and continuing reachability remain owner responsibilities.

The completed 0.3.18 direct-build, package, full-suite, render, publication, permanent-download, and installed-update checks are recorded in `QA_RESULTS.md`. These direct-channel checks do not satisfy the separate Apple signing, full-Xcode archive and App Store Connect prerequisites above.
...
Team and Bundle ID defaults are stored in `Config/AppStoreSigning.json`. `DABIN_DEVELOPMENT_TEAM` remains an optional explicit override and must match that checked-in configuration. The public URLs may be overridden with `DABIN_PRIVACY_POLICY_URL` and `DABIN_SUPPORT_URL`; otherwise the `DaBinPrivacyPolicyURL` and `DaBinSupportURL` Info.plist values are used. With full Xcode selected and the owner account added, run:

```sh
./scripts/archive_app_store.sh
```

The helper checks readiness inputs, injects those real URLs into the archive's Info.plist, and requests a uniquely named Release archive under `~/Library/Developer/Xcode/Archives/`. It rejects repository-contained, symbolic-link, wrong-extension, existing, and File Provider managed destinations. After Xcode archives, it runs the distribution-bundle preflight on the exact archived app. It does not enable automatic provisioning downloads, export, upload, submit, change keychain identities, or claim Apple approval. Distribution signing/export and validation are completed in Xcode Organizer using the owner's account. An exported candidate can additionally be inspected with:

```sh
python3 scripts/app_store_preflight.py --app /absolute/path/to/DaBin.app
```

This rejects ad-hoc/Developer ID signatures, the wrong configured team, non-ARM64/non-Release builds, missing App Sandbox, debugger entitlements, missing resources, missing release URLs, direct-update payloads, and quarantine metadata anywhere in the app bundle. It is a local check and cannot replace App Store Connect processing or review.

## Reviewer and listing notes to preserve

- DaBin opens Daily once on first launch, then stays quiet while idle. Its persistent menu bar item exposes Open Daily, Auto Capture state, Pause/Resume, Settings, and Quit. Screen corners are the default robot reveal target; Settings can move it below the built-in camera island when macOS exposes compatible safe-area geometry, and displays without one keep using corners. Double-click the robot to open Daily. Include these paths in review notes.
