...
GitHub published v0.3.18 as the latest normal release with all seven expected assets. Fresh anonymous downloads through every bare `/releases/latest/download/` URL matched the verified local files exactly, and the stable-alias workflow succeeded. The installed 0.3.17 app then found, downloaded, verified, backed up, and installed 0.3.18 through its own update UI. The installed executable matches the release build, the first Settings section is **Get updates**, and a follow-up check reports **You’re up to date with DaBin 0.3.18.** All 33 archive files, five originals, preferences, and the Desktop app link remained byte-identical.

[0.3.18 evidence](../docs/qa/0.3.18/README.md) · [Publication](../docs/qa/0.3.18/release-publication.json) · [Installed update](../docs/qa/0.3.18/live-update-v0.3.18.json) · [Full run](../docs/qa/0.3.18/full-run/report.json) · [Release renders](../docs/qa/0.3.18/release-ui-renders.json) · [Build receipt](../docs/qa/0.3.18/build-receipt-v0.3.18.json) · [Package comparison](../docs/qa/0.3.18/package-comparison-v0.3.18.json) · [Stable aliases](../docs/qa/0.3.18/stable-aliases-v0.3.18.json) · [Update manifest](../docs/qa/0.3.18/update-manifest-v0.3.18.json).
...
GitHub published v0.3.17 as the latest release. All five anonymously downloaded assets matched the verified local bytes, and the public one-click package independently passed isolated fresh-install, replacement, backup and signature execution. DaBin 0.3.16 then discovered and completed its own public update to **0.3.17 (42)**, created a signed backup, and reported that it was current. All 33 archive files, preferences, originals, imports and both Desktop links remained unchanged. The installed Settings control exposed **Quit DaBin completely** and its stable accessibility identifier; activating it left zero DaBin or updater processes. Finally, the separately published Apple Silicon ZIP was installed directly to `~/Applications`, passed strict signature and ARM64 checks, matched the build receipt, launched Daily, and again preserved the same local state.

[0.3.17 evidence](../docs/qa/0.3.17/README.md) · [Publication](../docs/qa/0.3.17/release-publication.json) · [Public installer](../docs/qa/0.3.17/public-update-installer-v0.3.17.json) · [Installed update](../docs/qa/0.3.17/live-update-v0.3.17.json) · [Complete quit](../docs/qa/0.3.17/live-quit-v0.3.17.json) · [Direct installation](../docs/qa/0.3.17/direct-install-v0.3.17.json) · [Full run](../docs/qa/0.3.17/full-run/report.json) · [Release renders](../docs/qa/0.3.17/release-ui-renders.json) · [Build receipt](../docs/qa/0.3.17/build-receipt-v0.3.17.json) · [Package comparison](../docs/qa/0.3.17/package-comparison-v0.3.17.json) · [Update manifest](../docs/qa/0.3.17/update-manifest-v0.3.17.json).
...
The optimized ARM64 build has production fingerprint `5f45db94220540928f16778d000e8e6dec2082260b4ca169cdd70a1b37dd4e86`. Its **5,984,832-byte** executable has SHA-256 `100a77fa2f09631fc3903f453e97f5bd8dc124a97fde50b5b59ee147eed0b158`. The verified **3,391,707-byte** update ZIP has SHA-256 `c938b966d6154f28f9c3273c4e0b577646ea7a3be50b120e56777d1085120992`; isolated package QA passed fresh installation, replacement, backup, signature, extraction, exact executable identity and document handoff. Static Store packaging passed **20/20** checks; Store release preflight retains the Apple Team ID and full-Xcode prerequisites.

GitHub published v0.3.16 as the latest release, and unauthenticated downloads of all four assets matched the local files exactly. The installed 0.3.15 app then completed its public self-update to **0.3.16 (41)**. The installed executable matched the release build, both app and updater passed strict deep signature checks, and the update download matched the published ZIP. Live QA selected Weekly and Daily in both directions, observed the delayed Weekly label under a real mouse event, confirmed the three nonempty days remained the only Weekly columns, and received **You’re up to date with DaBin 0.3.16.** All 33 archive files, every preference and the Desktop link remained unchanged; the updater created an exact signed backup of 0.3.15.

[0.3.16 evidence](../docs/qa/0.3.16/README.md) · [Publication](../docs/qa/0.3.16/release-publication.json) · [Installed update](../docs/qa/0.3.16/live-install-v0.3.16.json) · [Live packaged-app check](../docs/qa/0.3.16/local-live-mode-v0.3.16.json) · [Full run](../docs/qa/0.3.16/full-run/report.json) · [Release renders](../docs/qa/0.3.16/release-ui-renders.json) · [Build receipt](../docs/qa/0.3.16/build-receipt-v0.3.16.json) · [Update manifest](../docs/qa/0.3.16/update-manifest-v0.3.16.json).
...
This hotfix follows successful publication and in-app installation of 0.3.14. Live pointer QA of that installed build found that Add, Search, Export, Notifications and all six filters showed their labels, while Settings alone did not: the outer `Menu` intercepted hover before the inner label's callback. The issue did not affect opening or navigating the Settings menu.

The exact source passed **30/30 registered optimized Release suites** with no source changes during the final run. The header suite passed **39 checks**, every existing command remained functional, and **46 production-view renders** completed. The exact ARM64 Release app was then launched separately and received a real mouse-moved event over Settings: the short tooltip appeared after dwell, retained **Settings and options** as the accessibility name, stayed out of the accessibility tree itself, and the native menu opened afterward. The 33-file archive remained byte-identical.

The optimized build has production fingerprint `9678a70e2bb9ecc0e0e4a4df9f44d00dcbbe6c5eb3e5094126477e7aa3884192`. Its **5,987,296-byte** executable has SHA-256 `59a9db45185dc6ca8cba91bf82ae960acdceeae1f038bda0c0630a29afb14f8a`. The verified **3,391,579-byte** update ZIP has SHA-256 `b4e93f9d85147fc196b0a5e73038e70f4334003ac405fd5ad5323f7b96457b58`; isolated package QA passed fresh installation, replacement, backup, clean-copy signature checks, extraction, exact executable identity and document handoff. Static Store packaging passed **20/20** checks; Store release preflight retains the Apple Team ID and full-Xcode prerequisites.

Release [v0.3.15](https://github.com/RoeyAsterix/DaBin/releases/tag/v0.3.15) is public and latest. Its four unauthenticated downloads matched the verified ZIP, update manifest, PDF and release notes byte for byte. The installed 0.3.14 app then completed its public in-app update to 0.3.15. The installed executable and package hashes match the release; strict signatures, the verified 0.3.14 backup, all 33 archive files, preferences and the Desktop link passed. A real pointer dwell displayed **Settings**, the visual cue remained outside the accessibility tree, the menu opened, and a follow-up check reported that 0.3.15 is current.

[0.3.15 evidence](../docs/qa/0.3.15/README.md) · [Publication](../docs/qa/0.3.15/release-publication.json) · [Installed update](../docs/qa/0.3.15/live-install-v0.3.15.json) · [Live built-app hover](../docs/qa/0.3.15/local-live-hover-v0.3.15.json) · [Full run](../docs/qa/0.3.15/full-run/report.json) · [Build receipt](../docs/qa/0.3.15/build-receipt-v0.3.15.json) · [Update manifest](../docs/qa/0.3.15/update-manifest-v0.3.15.json).
...
Release [v0.3.14](https://github.com/RoeyAsterix/DaBin/releases/tag/v0.3.14) was published successfully and its four unauthenticated assets matched the verified local sources. The installed 0.3.13 app then completed its verified in-app update to 0.3.14; the installed executable matched the release build and all 33 archive files remained byte-identical. Subsequent live pointer QA found that the outer native Settings `Menu` intercepted hover before its inner gear label, so this one visual tooltip did not appear. The other ten tooltips and Settings menu behavior worked. Version 0.3.15 corrects that live-only gap.

[0.3.14 pre-publication evidence](../docs/qa/0.3.14/README.md) · [Publication](../docs/qa/0.3.14/release-publication.json) · [Live installation and pointer QA](../docs/qa/0.3.14/live-install-v0.3.14.json) · [Full optimized run](../docs/qa/0.3.14/full-run/report.json) · [Release renders](../docs/qa/0.3.14/release-ui-renders.json) · [Build receipt](../docs/qa/0.3.14/build-receipt-v0.3.14.json) · [Update manifest](../docs/qa/0.3.14/update-manifest-v0.3.14.json).
